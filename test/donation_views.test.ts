...
test("Finds first successful donations to email", async () => {
  ...
  const donation3 = await insertDonationMembershipViaQuickpay(db, {
    donor_id: donor2.id,
...
test("Should find donation ID by old donor ID", async () => {
  ...
  const oldDonation1 = await insertDonationMembershipViaQuickpay(db, {
    donor_id: oldDonor.id,
    method: PaymentMethod.CreditCard,
  });

  const oldDonation2 = await insertDonationMembershipViaQuickpay(db, {
    donor_id: oldDonor.id,
    method: PaymentMethod.CreditCard,
  });

  const newDonation1 = await insertDonationMembershipViaQuickpay(db, {
    donor_id: newDonor.id,
    method: PaymentMethod.CreditCard,
  });

  const newDonation2 = await insertDonationMembershipViaQuickpay(db, {
    donor_id: newDonor.id,
    method: PaymentMethod.CreditCard,
  });
