...
export async function getDonorIdsByEmail(
  ...
  return (
    await client.query(
      ...
      from donor
      where email = $1
      order by tin is null, created_at desc, id`,
      [email],
    )
  ).rows;
}
...
export async function getDonorsDetailedByEmail(
  ...
  return (
    await client.query(
      ...
      order by ds.tin is null, ds.created_at desc, ds.donor_id
      `,
      [email],
    )
  ).rows;
      ...
      with donations as (
        select
          case
            ...
            when t.recipient = 'Helen Keller International' then 'A-vitamin mod fejlernæring'
